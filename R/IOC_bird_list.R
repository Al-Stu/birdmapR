...
IOC.altnames <- function(formattedIOC, cleaned_redlist_data){
  ...
  all_scientific_names <- all_scientific_names[!is.na(all_scientific_names$scientificName),]
  all_scientific_names <- unique(all_scientific_names)
  duplicated_rows <- duplicated(all_scientific_names[c('internalTaxonId','scientificName')])
  dup_row_check <- all_scientific_names[duplicated_rows,]
  if(all(dup_row_check$main == 'false')){
    all_scientific_names <- all_scientific_names[duplicated_rows==FALSE,]
  } else {
